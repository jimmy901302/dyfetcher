...
class DouyinLiveWebFetcher:
    ...
    def start(self):
        # 在启动前先验证直播间号是否有效
        if not self.validate_live_id():
            raise ValueError(f"无效的直播间号：{self.live_id}")
    ...
    def room_id(self):
        """
        根据直播间的地址获取到真正的直播间 roomId，有时会有错误，可以重试请求解决
        :return:room_id
        """
        if self.__room_id:
    ...
    def validate_live_id(self) -> bool:
        """
        验证输入的直播间号是否有效
        :return: True 表示直播间号有效，False 表示无效
        """
        # 检查输入是否为空或不是数字
        if not self.live_id or not str(self.live_id).isdigit():
            print(f"【X】无效的直播间号：{self.live_id}，直播间号必须为数字")
            return False
            
        # 尝试获取 room_id，如果获取失败则说明直播间号无效
        try:
            rid = self.room_id
            if rid is None or rid == '':
                print(f"【X】直播间 {self.live_id} 不存在或已关闭")
                return False
            return True
        except Exception as e:
            print(f"【X】验证直播间号失败：{e}")
            return False
    ...
    def _parseGiftMsg(self, payload):
        ...
        print(f"【礼物消息】,{user_name},送出了,{gift_name}x{gift_cnt}")
        self._append_comment(
            "gift",
            user_id=message.user.id,
            user_name=user_name,
            content=f"{gift_name} x{gift_cnt}",
        )
    ...
    def _parseLikeMsg(self, payload):
        ...
        user_name = message.user.nick_name
        count = message.count
        print(f"【点赞消息】,{user_name},点了,{count},个赞")
        self._append_comment(
            "like",
            user_id=message.user.id,
            user_name=user_name,
    ...
    def _parseMemberMsg(self, payload):
        ...
        print(f"【进场消息】,[{user_id}],[{gender}],{user_name},进入了直播间")
        self._append_comment(
            "enter",
            user_id=user_id,
            user_name=user_name,
            content=f"[{gender}] 进入了直播间",
        )
    ...
    def _parseSocialMsg(self, payload):
        ...
        user_name = message.user.nick_name
        user_id = message.user.id
        print(f"【关注消息】,[{user_id}],{user_name},关注了主播")
        self._append_comment(
            "follow",
            user_id=user_id,
            user_name=user_name,
    ...
    def _parseRoomUserSeqMsg(self, payload):
        ...
        print(f"【统计消息】,当前观看人数: {current},累计观看人数,{total}")
        self._append_comment(
            "stats",
            content=f"当前观看人数: {current}, 累计观看人数: {total}",
            extra={
                "current": current,
                "total": total,
            },
    ...
    def _parseFansclubMsg(self, payload):
        ...
        self._append_comment("fansclub", content=content)
    ...
    def _parseEmojiChatMsg(self, payload):
        ...
        print(f"【聊天表情包消息】,{emoji_id},user：{user},common:{common},default_content:{default_content}")
        self._append_comment(
            "emoji",
            user_id=user.id,
    ...
    def _parseRoomMsg(self, payload):
        ...
        print(f"【直播间消息】,直播间id:{room_id}")
        self._append_comment(
            "room",
            content=f"直播间 id: {room_id}",
        )
    
    def _parseRoomStatsMsg(self, payload):
        message = RoomStatsMessage().parse(payload)
        display_long = message.display_long
        print(f"【直播间统计消息】,{display_long}")
        self._append_comment(
            "room_stats",
            content=display_long,
        )
    
    def _parseRankMsg(self, payload):
        message = RoomRankMessage().parse(payload)
        ranks_list = message.ranks_list
        print(f"【直播间排行榜消息】,{ranks_list}")
